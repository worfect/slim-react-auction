...
deploy:
	...
	ssh -o StrictHostKeyChecking=no root@${PORT} 'cd site_${BUILD_NUMBER} && docker-compose up -d --build  api-postgres api-php-cli'
	ssh -o StrictHostKeyChecking=no root@${PORT} 'cd site_${BUILD_NUMBER} && docker-compose run --rm api-php-cli wait-for-it api-postgres:5432 -t 60'
	ssh -o StrictHostKeyChecking=no root@${PORT} 'cd site_${BUILD_NUMBER} && docker-compose run --rm api-php-cli php bin/app.php migrations:migrate --no-interaction'
	ssh -o StrictHostKeyChecking=no root@${PORT} 'cd site_${BUILD_NUMBER} && docker-compose up -d --build --remove-orphans'
